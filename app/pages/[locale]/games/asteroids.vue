...
useHead({ title: `☄️ ${t('games.asteroids.title')}` });
</script>

<template>
  <div class="space-y-6">
    <h1 class="font-heading text-3xl font-bold text-[var(--foreground)]">☄️ {{ t('games.asteroids.title') }}</h1>
    <p class="font-body text-lg text-[var(--foreground)] opacity-70">
      {{ routeLocale === 'de' ? 'Asteroids kommt bald! 🎮' : 'Asteroids is coming soon! 🎮' }}
    </p>
  </div>
</template>