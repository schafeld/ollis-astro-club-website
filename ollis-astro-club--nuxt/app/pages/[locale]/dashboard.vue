...
<template>
  <div class="space-y-6">
    <h1 class="font-heading text-3xl font-bold text-[var(--foreground)]">🔒 {{ t('dashboard.title') }}</h1>
    <p class="font-body text-lg text-[var(--foreground)] opacity-70">
      {{ routeLocale === 'de' ? 'Der Benutzerbereich wird bald verfuegbar sein!' : 'The member area will be available soon!' }}
    </p>
  </div>
</template>