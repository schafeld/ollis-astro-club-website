...
useHead({ title: `🏎️ ${t('games.moonBuggy.title')}` });
</script>

<template>
  <div class="space-y-6">
    <h1 class="font-heading text-3xl font-bold text-[var(--foreground)]">🏎️ {{ t('games.moonBuggy.title') }}</h1>
    <p class="font-body text-lg text-[var(--foreground)] opacity-70">
      {{ routeLocale === 'de' ? 'Moon Buggy kommt bald! 🎮' : 'Moon Buggy is coming soon! 🎮' }}
    </p>
  </div>
</template>