...
useHead({ title: t('fun.videos') });
</script>

<template>
  <div class="space-y-6">
    <h1 class="font-heading text-3xl font-bold text-[var(--foreground)]">{{ t('fun.videos') }}</h1>
    <p class="font-body text-lg text-[var(--foreground)] opacity-70">
      {{ routeLocale === 'de' ? 'Videos kommen bald! 🎬' : 'Videos are coming soon! 🎬' }}
    </p>
  </div>
</template>